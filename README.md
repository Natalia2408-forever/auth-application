...
🧩 Description
This is an educational full-stack project implementing a complete authentication flow: registration with password confirmation, login/logout, JWT access + refresh tokens, and social login through Google and GitHub (OAuth via Passport.js). It also includes a small protected "Todos" page to demonstrate route protection after login.

🛠 Technologies

Backend
* Node.js / Express + TypeScript – REST API server and routing.
...
* Passport.js – Google and GitHub OAuth strategies.
...
Requirements: Node.js 20.19+ and a PostgreSQL database (local or hosted, e.g. Neon).
...
4. Frontend
   * cd frontend
   * npm install
   * copy .env.example to .env and set the VITE_API_URL variable
   * npm start
